...
Adapted from string-x's clc/googlecse.py. Instead of Google's official
Custom Search JSON API (which needs a billing-backed key), this drives the
CSE *widget* endpoint that any embedded search box uses, against a list of
third-party **public** CSE IDs shipped in assets/txt/google_cse_id.txt. That is
what makes Google coverage possible with zero credentials — and it works
where scraping google.com/search does not.
...
Each public CSE has its own index and scope, so rotating between a few IDs
returns different URLs for the same query.
...
# Public CSEs are a shared resource — a handful per run keeps the yield up
# without hammering any single one into rate-limiting it.
_IDS_PER_RUN = 3
_PAGES_PER_ID = 2
_PAGE_SIZE = 20
...
class Googlecse(BaseSource):
    NAME = 'googlecse'
    DESCRIPTION = 'Google via public Custom Search Engines — indexed URLs, no API key'
    API_TOKEN_IS_REQUIREMENT = False

    async def fetch(self, target: Target) -> set[str]:
        cse_ids = self._load_ids()
        if not cse_ids:
        ...
        try:
            async with self._make_client(
                ...
                for cx in chosen:
                    try:
                        found = await self._search_with(client, cx, dork)
                    except Exception as e:
                        self._log_exc(e)
                        continue  # dead/blocked CSE — expected, try the next
                    if found:
    ...
    def _load_ids() -> list[str]:
        return list(load_lines(_CSE_ID_FILE))

    async def _search_with(self, client, cx: str, dork: str) -> set[str]:
        """Run the two-step CSE flow for one CSE ID."""
        ...
        found: set[str] = set()
        for page in range(_PAGES_PER_ID):
            start = page * _PAGE_SIZE
            # `rurl` is intentionally nested and unencoded — see module docstring.
            rurl = f'https://cse.google.com/cse?cx={cx}&q={dork}&num=500&hl=en&as_qdr=all&start={start}&sa=N'
            url = (
                f'{_ELEMENT}?rsz=filtered_cse&num={_PAGE_SIZE}&hl=en&source=gcsc'
                f'&start={start}&cselibv={libver.group(1)}&cx={cx}&q={quote(dork)}'
