...
# -*- coding: utf-8 -*-
# SimpleReconURL v1
# https://github.com/osintbrazuca/SimpleReconURL
#
# WARNING
# +------------------------------------------------------------------------------+
# |  [!] Legal disclaimer: Usage of SimpleReconURL for attacking                 |
...
def _banner_footer() -> str:
    ...
    """
    return '\n'.join([
        f"    {colors.bold(f'SimpleReconURL v{settings.__version__}')} "
...
def main() -> None:
    ...
    if banner_ok and (args.list_sources or args.list_profiles or args.list_examples):
        _print_banner()
    ...
    if getattr(args, 'db_list', None):
        ...
        if args.db_list != 'history' and not getattr(args, 'db', None):
            ...
            sys.exit(1)
        print_db_list(args.db_list, getattr(args, 'db', None), args.url)
        sys.exit(0)
    ...
    if not args.url and not args.list and not getattr(args, 'stdin', False) and sys.stdin.isatty():
        if banner_ok:
            _print_banner()
        parser.print_help()
        sys.exit(1)
