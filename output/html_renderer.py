"""
HTML renderer for the page-link graph visualization.

Produces a self-contained HTML file that loads vis-network from a CDN and
renders the per-target graphs (built by output/graph.py) as an interactive
node/edge diagram.
"""
...
<head>
<meta charset="utf-8">
<title>SimpleReconURL — Page-Link Map ({targets})</title>
...
<div id="wrap">
  <aside id="side">
    ...
    <h2>Nodes</h2>
    <ul id="legend-nodes">
      <li><span class="dot" style="background:#1976d2"></span>seed<span class="count">{c_seeds}</span></li>
      <li><span class="dot" style="background:#9e9e9e"></span>page<span class="count">{c_pages}</span></li>
      <li><span class="dot" style="background:#757575"></span>external<span class="count">{c_externals}</span></li>
    </ul>

    <h2>Page status</h2>
    <ul>
      <li><span class="dot" style="background:#4caf50"></span>2xx</li>
      <li><span class="dot" style="background:#ff9800"></span>3xx</li>
      <li><span class="dot" style="background:#f44336"></span>4xx</li>
      <li><span class="dot" style="background:#9c27b0"></span>5xx</li>
      <li><span class="dot" style="background:#9e9e9e"></span>none / unreachable</li>
    </ul>

    <h2>Edges</h2>
    <ul>
      <li>links_to<span class="count">{c_edges}</span></li>
    </ul>
  </aside>
  <main id="net"></main>
  <div id="info"></div>
...
const nodes = new vis.DataSet(GRAPH.nodes.map(n => ({{
  ...
  shape: ({{ seed: 'star', page: 'dot', external: 'triangle' }})[n.type] || 'dot',
  size: n.type === 'seed' ? 26 : (n.type === 'page' ? 14 : 10),
...
network.on('click', params => {{
  ...
  if (m.status !== undefined && m.status !== null) parts.push(`<span class="pill">HTTP ${{m.status}}</span>`);
  if (m.title) parts.push(`<pre>${{m.title}}</pre>`);
  if (m.server) parts.push(`<pre>${{m.server}}</pre>`);
...
def render_html(results: list[dict]) -> str:
    ...
    Builds a per-target graph for each result, merges them into one combined
    view, then embeds the JSON payload into a vis-network template.
    """
    per_target = [build_network_graph(r) for r in results]
    merged = merge_graphs(per_target)
    ...
    return _TEMPLATE.format(
        targets=html.escape(', '.join(target_names) or 'page-link map'),
        targets_html=targets_html,
        ts=datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        c_seeds=stats.get('seeds', 0),
        c_pages=stats.get('pages', 0),
        c_externals=stats.get('externals', 0),
